...
;; Added quote, map and and functions in eval-1
;; Nathan MacDiarmid
;; 101098993
...
(define (eval-1 exp)
  (cond ((constant? exp) exp)
    ...
    ((map-exp? exp)
         (cond
           [(= (length exp) 3) (map (eval (cadr exp)) (eval (caddr exp)))] ;; evaluates the expression using the built in map function if theres one list
           [(= (length exp) 4) (map (eval (cadr exp)) (eval (caddr exp)) (eval (cadddr exp)))] ;; evaluates the expression using the built in map function if theres two lists
           ))
    ;; Added the and-exp? to evaluate an and function
    ((and-exp? exp)
     (eval exp) ;; evaluates the expression witht the built in and function
     )
    ((if-exp? exp)
     (if (eval-1 (cadr exp))
         (eval-1 (caddr exp))
